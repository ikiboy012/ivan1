...
export function createAsyncStorageHelpers<AsyncStorageType>(
    ...
    async function setAsyncStorage<T extends keyof AsyncStorageType>(key: T, value: AsyncStorageType[T]) {
        try {
    ...
    async function getAsyncStorage<T extends keyof AsyncStorageType>(key: T) {
        try {
            ...
            return jsonValue != null ? (JSON.parse(jsonValue) as AsyncStorageType[T]) : asyncStorageDefaults[key];
        } catch (err) {
            console.warn(err);
            return null;
        }
